...
import { carriers, circuits, clients, dids, hostingProviders, linepbxSettings, newId, products, subscriptions, type Db } from '@gestor/db';
...
const ALIASES: Record<Entity, Record<string, string[]>> = {
  clients: {
    ...
    archived: ['arquivado', 'archived', 'oculto', 'is_occult'], products: ['produtos', 'products'], notes: ['observacoes', 'notes', 'anotacoes'],
  ...
  circuits: {
    ...
    authPassword: ['senha_auth', 'auth_password', 'senha'], monthlyValue: ['valor', 'monthly_value', 'valor_mensal'], owner: ['dono', 'owner', 'proprietario'], notes: ['observacoes', 'notes'],
  },
  dids: {
    ...
    owner: ['dono', 'owner', 'proprietario'], note: ['observacao', 'note', 'obs'],
  },
};
...
async function planClients(db: Db, raw: Record<string, string>[]): Promise<Plan> {
  ...
  const rows: PlanRow[] = raw.map((r, i) => {
    ...
    const prodList = (pick(r, 'clients', 'products') ?? '').split(/[|,]/).map((p) => norm(p)).filter(Boolean);
    ...
    const data = {
      ...
      products: prodList, domain: pick(r, 'clients', 'domain'), serverIp: pick(r, 'clients', 'serverIp'), sshUser: pick(r, 'clients', 'sshUser'),
...
async function planCircuits(db: Db, raw: Record<string, string>[]): Promise<Plan> {
  ...
  const rows: PlanRow[] = raw.map((r, i) => {
    ...
    if (ownerRaw && !owner) errors.push(`Dono desconhecido: ${ownerRaw}`);
...
export async function exportCsv(db: Db, vault: SecretsVault, entity: Entity, withSecrets: boolean): Promise<{ filename: string; csv: string }> {
  ...
  if (entity === 'clients') {
    ...
    for (const c of rows) {
      ...
      out.push({
        id: c.id, cnpj: c.cnpj, nome_fantasia: c.tradeName, razao_social: c.legalName, arquivado: c.archived ? 'sim' : 'nao', produtos: mine.map((s) => s.code).join('|'),
  ...
  if (entity === 'circuits') {
    ...
    for (const r of rows) {
      out.push({
        ...
        valor: r.c.monthlyValueCents != null ? (r.c.monthlyValueCents / 100).toFixed(2).replace('.', ',') : '', dono: r.ownerName ?? '', observacoes: r.c.notes ?? '',
      });
    }
  ...
  const out = rows.map((r) => ({ numero: r.number, circuito: r.circuitCode ?? '', circuito_nome: r.circuitName ?? '', cliente: r.clientCnpj ?? '', cliente_nome: r.clientName ?? 'livre', dono: r.ownerId ? owners.get(r.ownerId) ?? '' : '', observacao: r.note ?? '' }));
