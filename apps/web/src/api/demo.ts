...
type DeviceRow = { id: string; modelId: string; mac: string; macSecondary: string | null; tag: string | null; clientId: string | null; unit: string | null; currentModality: string | null; condition: string; valueCents: number | null; ip: string | null; location: string | null; note: string | null; deletedAt: string | null; createdAt: string };
...
function seed() {
  ...
  for (const [cli, q, d] of locados) {
    ...
    for (let i = 0; i < q; i++, n++) { const dev: DeviceRow = { id: id(), modelId: mGx.id, mac: '000B82' + (0x100000 + n).toString(16).toUpperCase().slice(-6), macSecondary: null, tag: null, clientId: byName[cli]!, unit: UNIDADES_DEMO[cli]?.[i % (UNIDADES_DEMO[cli]!.length)] ?? null, currentModality: 'locacao', condition: 'ativo', valueCents: 45000, ip: `10.0.${n % 4}.${20 + n}`, location: null, note: null, deletedAt: null, createdAt: daysAgo(d + 3) }; S.devices.push(dev); items.push({ modelId: mGx.id, deviceId: dev.id, quantity: 1 }); }
  ...
  for (let i = 0; i < 12; i++, n++) S.devices.push({ id: id(), modelId: mGx.id, mac: '000B82' + (0x100000 + n).toString(16).toUpperCase().slice(-6), macSecondary: null, tag: null, clientId: null, unit: null, currentModality: null, condition: i === 11 ? 'manutencao' : 'ativo', valueCents: 45000, ip: null, location: 'Estoque · Prateleira B', note: i === 11 ? 'Tela piscando; aguardando peça' : null, deletedAt: null, createdAt: daysAgo(60) });
  for (let i = 0; i < 3; i++, n++) S.devices.push({ id: id(), modelId: mTip.id, mac: '1C61B4' + (0x200000 + n).toString(16).toUpperCase().slice(-6), macSecondary: null, tag: null, clientId: null, unit: null, currentModality: null, condition: 'ativo', valueCents: 38000, ip: null, location: 'Estoque · Prateleira A', note: null, deletedAt: null, createdAt: daysAgo(20) });
...
export const demoApi: Api = {
  ...
  dashboard: {
    ...
    async search(q) {
      ...
      return {
        ...
        devices: S.devices.filter((d) => !d.deletedAt && ((hex.length >= 4 && d.mac.includes(hex)) || (d.unit ?? '').toLowerCase().includes(t))).slice(0, 8).map((d) => ({ id: d.id, mac: d.mac, macFormatted: macFormatado(d.mac), tag: d.tag, modelName: S.models.find((m) => m.id === d.modelId)?.name ?? '?', clientName: S.clients.find((x) => x.id === d.clientId)?.tradeName ?? null })),
      };
    },
  },
  clients: {
  ...
  inventory: {
    ...
    async createDevice(d) { await wait(); requirePerm('records.write'); const m = S.models.find((x) => x.id === d.modelId); if (!m) throw notFound('Modelo'); if (m.tracking !== 'serializado') throw bad('Este modelo é contado a granel; use "ajustar estoque"'); const mac = macLimpo(String(d.mac)); if (mac.length !== 12) throw new ApiError(400, 'Dados inválidos', [{ field: 'mac', message: 'MAC precisa ter 12 caracteres hexadecimais' }]); if (S.devices.some((x) => x.mac === mac)) throw bad(`Já existe um aparelho com o MAC ${macFormatado(mac)}`); const row: DeviceRow = { id: id(), modelId: m.id, mac, macSecondary: d.macSecondary ? macLimpo(String(d.macSecondary)) : null, tag: null, clientId: null, unit: null, currentModality: null, condition: String(d.condition ?? 'ativo'), valueCents: (d.valueCents as number) ?? null, ip: (d.ip as string) ?? null, location: (d.location as string) ?? null, note: (d.note as string) ?? null, deletedAt: null, createdAt: now() }; S.devices.push(row); audit('create', 'device', `Cadastrou o aparelho ${macFormatado(mac)}`, row.id); return shapeDevice(row, true); },
