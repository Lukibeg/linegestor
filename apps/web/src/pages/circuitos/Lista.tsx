/** Circuitos: lista com ocupação e formulário de criar/editar. */
...
import { Campo, CampoSegredo, Carregando, Modal, Ocupacao, Paginacao, Spinner, Vazio, mensagemErro, useToast } from '../../components/ui/index.js';
...
export function CircuitosLista() {
  ...
  const lista = useQuery({ queryKey: ['circuits', q, carrierId, page], queryFn: () => api.circuits.list({ q, carrierId, page, pageSize: 50 }) });
  const totais = lista.data ? lista.data.items.reduce((a, c) => ({ dids: a.dids + c.dids.total, free: a.free + c.dids.free, ch: a.ch + c.channels }), { dids: 0, free: 0, ch: 0 }) : null;
  return (
    <Pagina titulo="Circuitos" sub={totais ? `${lista.data!.total} circuito(s) · ${totais.ch} canais · ${totais.dids} DIDs (${totais.free} livres)` : ' '} acoes={<Can permission="records.write"><button className="btn-primary" onClick={() => setNovo(true)}><Plus size={16} /> Novo circuito</button></Can>}>
      ...
      {lista.isLoading ? <Carregando /> : !lista.data?.items.length ? <Vazio titulo="Nenhum circuito" texto="Cadastre o feixe contratado junto à operadora." /> : (
        <div className="card overflow-x-auto"><table className="table">
          <thead><tr><th>Nome</th><th>Operadora</th><th>Código</th><th className="text-right">Canais</th><th className="text-right">DIDs</th><th className="text-right">Livres</th><th>Ocupação</th><th className="text-right">Valor/mês</th></tr></thead>
          <tbody>{lista.data.items.map((c) => (
            <tr key={c.id} className="cursor-pointer" onClick={() => nav(`/circuitos/${c.id}`)}>
              <td className="font-medium">{c.name}</td><td>{c.carrierName ?? '—'}</td><td className="font-mono tnum">{c.code}</td><td className="text-right tnum">{c.channels}</td><td className="text-right tnum">{c.dids.total}</td><td className="text-right tnum">{c.dids.free}</td><td><Ocupacao total={c.dids.total} channels={c.channels} /></td><td className="text-right tnum">{reais(c.monthlyValueCents)}</td>
            </tr>))}</tbody></table></div>
      )}
    ...
    <Modal open={open} onClose={onClose} lateral largura="max-w-xl" titulo={circuito ? `Editar ${circuito.name}` : 'Novo circuito'} rodape={<><button className="btn-secondary" onClick={onClose}>Cancelar</button><button className="btn-primary" disabled={busy || !f.name || !f.code} onClick={save}>{busy ? <Spinner className="text-white" /> : 'Salvar'}</button></>}>
      <div className="flex flex-col gap-3">
        ...
        <div className="grid grid-cols-2 gap-3">
          ...
          <Campo label="Dono" className="col-span-2"><select className="input" value={f.ownerClientId ?? ''} onChange={(e) => setF({ ...f, ownerClientId: e.target.value })}><option value="">—</option>{owners.data?.map((o) => <option key={o.id} value={o.id}>{o.name}{o.isInternal ? ' (interna)' : ''}</option>)}</select></Campo>
        </div>
