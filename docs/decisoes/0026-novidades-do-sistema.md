...
**Decisão.** Uma seção **Novidades**, no menu logo abaixo de Inventário.
...
- **Rascunho x publicada.** A nota nasce rascunho e ninguém a vê. **Publicar** é um botão à parte
  ...
  menos um item.
- **Só a mais recente abre no login.** Quem entra pela primeira vez não leva as notas antigas na
  cara; as anteriores ficam na página. Fechar sem marcar não conta — volta no próximo acesso.
  A nota some de vez quando a pessoa marca **"Li e entendi"** no último cartão.
...
- **A nota de cada rodada chega pronta.** Eu escrevo em `docs/novidades/<versao>.md` (cabeçalho com
...
- 8 testes novos da API (66 no total) cobrem rascunho, publicação, "só a mais recente abre",
