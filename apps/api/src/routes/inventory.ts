...
import { z } from 'zod';
import { AparelhoAtualizarSchema, AparelhoGravarSchema, MODALIDADES, ModeloGravarSchema, MovimentacaoCriarSchema, OrdenacaoSchema, PaginacaoSchema } from '@gestor/shared';
...
const routes: FastifyPluginAsyncZod = async (app) => {
  // ---- modelos ----
  app.get('/models', { preHandler: app.requirePermission('records.read'), schema: { tags: ['Inventário'], summary: 'Modelos com contagens (em estoque, com clientes, inativos, vendidos)', querystring: z.object({ q: z.string().optional(), categoryId: z.string().optional() }) } },
  ...
  app.post('/devices', { preHandler: app.requirePermission('records.write'), schema: { tags: ['Inventário'], summary: 'Cadastrar aparelho (entra no estoque)', body: AparelhoGravarSchema } },
    async (req, reply) => { const row = await svc.createDevice(app.db, req.body); await app.audit(req, { action: 'create', entityType: 'device', entityId: row.id, summary: `Cadastrou o aparelho ${row.mac ?? row.modelId}`, after: row }); return reply.status(201).send(row); });
  app.patch('/devices/:id', { preHandler: app.requirePermission('records.write'), schema: { tags: ['Inventário'], summary: 'Editar aparelho (condição, valor, IP, local, anotação)', params: Id, body: AparelhoAtualizarSchema } },
    async (req) => { const { before, after } = await svc.updateDevice(app.db, req.params.id, req.body); await app.audit(req, { action: 'update', entityType: 'device', entityId: after.id, summary: `Editou o aparelho ${after.mac ?? after.id}`, before, after }); return svc.getDevice(app.db, after.id); });
  app.delete('/devices/:id', { preHandler: app.requirePermission('records.delete'), schema: { tags: ['Inventário'], summary: 'Mandar aparelho para a lixeira', params: Id } },
    async (req) => { const row = await svc.deleteDevice(app.db, req.params.id); await app.audit(req, { action: 'delete', entityType: 'device', entityId: row.id, summary: `Mandou o aparelho ${row.mac ?? row.id} para a lixeira` }); return { ok: true }; });
  app.post('/devices/:id/restore', { preHandler: app.requirePermission('records.delete'), schema: { tags: ['Inventário'], summary: 'Restaurar aparelho', params: Id } },
    async (req) => { const row = await svc.restoreDevice(app.db, req.params.id); await app.audit(req, { action: 'restore', entityType: 'device', entityId: row.id, summary: `Restaurou o aparelho ${row.mac ?? row.id}` }); return svc.getDevice(app.db, row.id); });
